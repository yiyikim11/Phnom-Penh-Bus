  ...
  if (valid) {
    for (let i = 0; i < fromTimes.length; i++) {
      ...
      if (arrivalTime) {
        ...
        if (arrivalDate.getTime() <= currentTime.getTime()) {
          alert('The arrival time has already passed, please change the bus arrival time.');
          return; // Stop further processing if the arrival time has passed.
        }
    ...
    alert(`Your trip plan has been added successfully! 
      Please kindly check your email to complete your payment and further confirmation. 
